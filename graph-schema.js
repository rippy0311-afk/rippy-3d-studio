...
export const TYPES={
 setup:make('最初の定義','初期設定',{player:0,movers:[],view:'back',forward:'-z',controls:'both',speed:4,distance:6,height:1,eyeHeight:.7,sensitivity:1,turn:true,collision:true,sprintOn:true,runSpeed:7,jumpOn:true,jumpSpeed:6,gravityOn:false,gravityStrength:18,stepHeight:.35,maxSlope:50},[f('player','プレイヤー（0＝なし）','playerTarget'),f('movers','追加の操作対象（複数選択）','targets'),f('view','プレイヤー視点','select',[['三人称（後方）','back'],['三人称（前方）','front'],['一人称','first']]),f('forward','プレイヤーの前（ローカル軸）','select',[['−Z','-z'],['＋Z','+z'],['＋X','+x'],['−X','-x']]),f('controls','移動キー','select',[['WASD＋矢印','both'],['WASD','wasd'],['矢印','arrows'],['なし（ノードで移動）','none']]),f('speed','歩く速度 m/秒','setting'),f('sprintOn','Shiftで走る','boolean'),f('runSpeed','走る速度 m/秒','setting'),f('jumpOn','Spaceでジャンプ','boolean'),f('jumpSpeed','ジャンプの強さ m/秒','setting'),f('gravityOn','重力・地面判定を有効にする','boolean'),f('gravityStrength','重力 m/秒²','setting'),f('stepHeight','乗り越える段差 m','setting'),f('maxSlope','登れる坂の角度 °（最大85）','setting'),f('distance','三人称の距離 m','setting'),f('height','三人称の注視点の高さ m','setting'),f('eyeHeight','一人称の目の高さ m','setting'),f('sensitivity','視点の感度','setting'),f('turn','移動方向にプレイヤーを向ける','boolean'),f('collision','形どうしの衝突を有効にする','boolean')],{event:true}),
